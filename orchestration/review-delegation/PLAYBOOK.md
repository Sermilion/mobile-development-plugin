...
- Every delegated worker must receive the exact review scope, changed files or diff source, relevant project guidance, the delegated skill file path, the current `review_run_id` when one already exists, and the shared contract from `review-orchestrator.md`.
