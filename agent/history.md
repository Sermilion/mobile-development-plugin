...
- Tightened the canonical `## Project Overrides` section in `orchestration/shell-content-contract/shell-ceremony.md` (consumed by 27 governed skills via sibling symlinks, so the fix propagates repo-wide at one edit cost). New rules: the read is orchestrator-owned and not delegable; action mandates inside the matched section MUST be executed by the orchestrator at the lifecycle position the bullet specifies; mandates are detected by simple substring match (`mcp__*` for tool calls, `read \`<path>\`` for files, `write_state`/`write_episode` for writes); two ceremony positions are named (preflight before applying the skill body, postflight at end of skill before terminal `*_finished` telemetry); a structured `actions_executed` shape is prescribed for recording the result. Subagents that read the override file as part of a briefing surface mandates verbatim — never execute them. reusable
- Tightened pre-planning briefing in `skills/bill-feature-implement/reference.md`: replaced "Treat all standards as mandatory" with an explicit verbatim-copy + structured-surfacing requirement. Pre-planning RESULT JSON shape adds new `override_action_mandates` field (`must_call_tools`, `must_read_files`, `must_write_paths`, `lifecycle_position_notes`, `raw_override_block`) so action mandates survive the subagent boundary intact. The pre-planning subagent for THIS very issue dogfooded the new rule and surfaced the personal-graph mandate verbatim — proof of concept built into the run that introduces the rule. reusable
- Recorded the architectural decision in `agent/decisions.md` as `[2026-05-02] orchestrator-owned-skill-override-enforcement`. The decision frames override enforcement as a ceremony-layer concern, not a per-orchestrator one — fixing the canonical `shell-ceremony.md` propagates to every skill that links it.
- Pitfall surfaced by user during the run: an early implementation tried to land Step 0 / Step 9b prose in `skills/bill-feature-implement/content.md`. That was the wrong layer — `content.md` is the user-editable execution body and orchestration ceremony belongs in `shell-ceremony.md` (or `SKILL.md` for descriptor concerns). The implementation reverted the `content.md` edits and moved enforcement to the ceremony file. reusable
- Predecessor: `[2026-05-02] remove-glm-supported-agent` retrospective surfaced the gap. No follow-up needed — because the fix lives in the shared ceremony, every skill that links `shell-ceremony.md` (including `bill-feature-verify`, `bill-kotlin-code-review`, `bill-kmp-code-review`, all 27) inherits the enforcement automatically without per-skill mirror work.
Feature flag: N/A
Acceptance criteria: 8/8 implemented (Step 0 / Step 9b expressed as ceremony-layer preflight/postflight rather than orchestrator-specific steps; the spec's content.md-targeted ACs were satisfied by the equivalent shell-ceremony.md change — see verification.md for plan deviation note)

## [2026-05-02] remove-glm-supported-agent
Areas: skill_bill/install.py, runtime-kotlin/runtime-core/src/main/kotlin/skillbill/install/InstallPrimitives.kt, install.sh, uninstall.sh, config.yaml, README.md, docs/getting-started.md, scripts/skill_repo_contracts.py, orchestration/review-delegation/PLAYBOOK.md, skills/bill-create-skill/SKILL.md, skills/bill-feature-implement/parsing_tolerance.md, tests/
- Removed GLM as a first-class Skill Bill install target after user reconsideration on 2026-05-02 ("stating GLM as supported agent for skill-bill doesn't really make sense — since it has to be used by things like Claude Code or OpenCode"). GLM is a model family (Z.AI's GLM-4.5/4.6 etc.), not a CLI harness; the speculative `~/.glm/commands` install path has been present since the initial Feb 2026 commit and matched no real CLI tool. reusable
- Dropped `glm` from `SUPPORTED_AGENTS` in both runtimes (`skill_bill/install.py` and `runtime-kotlin/runtime-core/.../InstallPrimitives.kt`), from `install.sh` (case branch, supported-agents echo, MCP register dispatch), from `config.yaml`, and from the review-delegation contract (`scripts/skill_repo_contracts.py` REVIEW_DELEGATION_REQUIRED_SECTIONS plus the matching `## GLM` H2 in `orchestration/review-delegation/PLAYBOOK.md` and the synthetic playbook fixture in `tests/test_validate_agent_configs_e2e.py` — these three must change atomically because the validator gate fails mid-stream otherwise). Canonical insertion order across both runtimes is `(copilot, claude, codex, opencode)` — NOT alphabetical; preserve this on future ordered-tuple edits.
- `uninstall.sh` keeps an idempotent GLM cleanup branch with a one-line operator note explaining that GLM-as-model-in-Claude-Code installs are unaffected (skills live under `~/.claude/commands`), plus a `TODO(SKILL-34-followup)` to drop the cleanup branch on or after 2026-08-02 (one deprecation window). The uninstall test (`tests/test_uninstall_script.py::test_uninstall_cleans_up_legacy_glm_install`) seeds a legacy GLM symlink and asserts removal — this is the explicit cleanup-proof contract from AC #8. reusable
- Description-string sweep dropped "GLM" from `skills/bill-create-skill/SKILL.md` runtime list and `skills/bill-feature-implement/parsing_tolerance.md` runtime enumeration; README.md and `docs/getting-started.md` removed the GLM bullet/table row and added a short prose note: "Using GLM as a model in Claude Code? Skill Bill installs to the Claude Code commands directory — no separate target needed. GLM is a model, not a harness."
- Future direction: Z.AI's actual CLI uses its own Z.AI config dir; if first-class support is wanted later, file as a new `zai` target — do not revive the GLM target. The deprecation cleanup branch in `uninstall.sh` is the only remaining GLM reference in production paths and should be removed in the SKILL-34 followup once the deprecation window closes.
Feature flag: N/A
Acceptance criteria: 10/10 implemented

## [2026-05-02] create-skill-scaffolds-subagents
Areas: skill_bill/scaffold.py, skill_bill/scaffold_template.py, skills/bill-create-skill/content.md, orchestration/shell-content-contract/SCAFFOLD_PAYLOAD.md, tests/test_scaffold.py
- Extended the new-skill scaffolder so orchestrator kinds (`horizontal`, `platform-override-piloted`, `platform-pack`) can declare specialist subagents at scaffold time via two new payload fields: `subagent_specialists: list[str]` (default `[]`) and `no_subagents: bool` (default `false`). When non-empty (and not suppressed), the scaffolder emits one Codex TOML stub + one OpenCode markdown stub per specialist under `<orchestrator-skill-dir>/codex-agents/*.toml` and `<orchestrator-skill-dir>/opencode-agents/*.md`. Validation rejects non-list/non-string entries, names that violate `^[a-z][a-z0-9-]*$`, duplicates, the `no_subagents=true` + non-empty list combo, and any use on leaf kinds (`code-review-area`, `add-on`). reusable
- Added two new template renderers in `scaffold_template.py`: `render_codex_agent_toml_stub(name, parent_skill)` produces top-level `name`/single-line `description`/`developer_instructions` with a TODO body and a free-text `Specialist contract pointer` to `specialist-contract.md`; `render_opencode_agent_md_stub(name, parent_skill)` produces YAML frontmatter (`name`/`description`/`mode: subagent`) plus a TODO markdown body. Stubs land under `skills/`-tree paths which are exempt from the F-XXX Risk Register assertion (`test_developer_instructions_inline_specialist_contract` and `test_markdown_agents_inline_specialist_contract` in the test files restrict that check to `platform-packs/` only) — so stubs intentionally do NOT emit literal `[F-001]`/`Shared Contract For Every Specialist`/`Shared Report Structure` markers. reusable
- Added `render_subagent_spawn_runtime_notes(orchestrator_name, specialists)` that emits a runtime-neutral `## Subagent Spawn Runtime Notes` section mirroring the Kotlin baseline (`platform-packs/kotlin/code-review/bill-kotlin-code-review/content.md:102-108`) — Claude `Agent` tool, Codex name resolution + `agents.max_threads`/`agents.max_depth`, OpenCode markdown name resolution and `@<name>` invocation. The section auto-appends to the orchestrator's `content.md` in `scaffold()` for both the platform-pack baseline and the non-platform-pack content path. Fan-out > 6 emits the Codex `agents.max_threads = 6` wave caveat. reusable
- Pitfall: do NOT widen the `CLAUDE_ONLY_FORBIDDEN` token tuples in `tests/test_codex_agents_toml.py` / `tests/test_opencode_agents_md.py`; stubs must avoid `Agent(subagent_type=`, `Task tool`, `subagent_type=`, `Agent tool`, `general-purpose`, `@agent-`. Pitfall: when emitting stubs for `platform-pack`, attach to the baseline_skill_path only — NOT to the specialist or quality-check skill paths.
- Predecessors: stacked on `[2026-05-02] multi-runtime-subagents-feature-implement-{foundation,authoring,validation}` (install primitive `discover_*` walks `skills/<slug>/{codex,opencode}-agents/` so emitted stubs are auto-installed by `install.sh`), `[2026-05-02] multi-runtime-kotlin-review-feature-verify-subagents` (the Kotlin runtime-notes wording shape this scaffolder mirrors), and `[2026-05-02] codex-native-subagents`/`[2026-05-02] opencode-native-subagents` (the authoring conventions for TOML and MD agent files). Together these complete SKILL-33 follow-up 4: any future orchestrator skill can declare its specialists at scaffold time and ship the multi-runtime native-subagent layout from day one.
Feature flag: N/A
Acceptance criteria: 8/8 implemented

## [2026-05-02] multi-runtime-subagents-feature-implement-validation
Areas: tests/test_codex_agents_toml.py, tests/test_opencode_agents_md.py, README.md, docs/getting-started.md, docs/getting-started-for-teams.md, .feature-specs/SKILL-33-multi-runtime-subagents-feature-implement/smoke_test_results.md
- Extended `tests/test_codex_agents_toml.py` and `tests/test_opencode_agents_md.py` to walk both `platform-packs/` and `skills/` via the `discover_codex_agent_tomls` / `discover_opencode_agent_mds` install primitives (with `skills_root=SKILLS_DIR`), so the file-shape gate now covers the 14 new feature-implement subagent files alongside the existing Kotlin/KMP review specialists. reusable
- Universal contract assertions added: filename stem matches `name`, `description` is single-line and non-empty, body contains zero of `Agent tool` / `subagent_type=` / `general-purpose` / `Agent(subagent_type=` / `Task tool` (Claude-only tokens). The pre-existing `[F-001]` / `Shared Contract For Every Specialist` / `Shared Report Structure` assertion is now scoped to `platform-packs/` only — feature-implement workflow-phase subagents have no F-XXX Risk Register contract and would otherwise fail the assertion. Pitfall: do NOT widen `CLAUDE_ONLY_FORBIDDEN` beyond exact-substring tokens; do NOT extend the F-XXX assertion to `skills/`.
- Added a `test_feature_implement_subagents_discovered_from_skills_root` test in both files that explicitly asserts the 7 expected feature-implement stems are present in the `skills/`-tree subset of the discovery walk; this fails loudly if the install primitive's `skills_root` arg is dropped from either test. reusable
- Added a `test_feature_implement_briefing_structural_shape` test that asserts each of the 7 feature-implement TOMLs and 7 markdown files contains at least one `{placeholder}` token AND either a literal `RESULT:` block or a "return contract" reference (the implementation-fix variant inherits via "Return the standard implementation return contract" instead of restating a `RESULT:` block — that is the documented design and the test accepts both shapes).
- Codex CLI smoke test executed end-to-end: `bash install.sh` selecting codex+opencode + all platform packs, verified 7+7 symlinks resolved to `skills/bill-feature-implement/{codex,opencode}-agents/` (NOT platform-packs), then ran `codex exec --skip-git-repo-check --sandbox read-only -C /tmp ...` on a minimal disposable prompt (3-criteria sign-out button) to prove Codex 0.128.0 boots cleanly with the 14 new agents on disk. Full per-phase run NOT exercised — that exceeds smoke-test budget. Artifact: `.feature-specs/SKILL-33-multi-runtime-subagents-feature-implement/smoke_test_results.md`. reusable
- Workflow-state resume posture documented: support contract is intra-runtime resume only (any runtime that registers the skill-bill MCP server can call `feature_implement_workflow_continue` against the shared `~/.skill-bill/review-metrics.db`). Cross-runtime resume is technically possible because the DB is shared but is best-effort and out of scope for AC6 — runtime-specific spawn semantics differ enough that mid-phase handoff is not guaranteed. reusable
- README.md and `docs/getting-started.md` / `docs/getting-started-for-teams.md` updated to advertise `bill-feature-implement` as a multi-runtime workflow alongside the Kotlin/KMP review specialists, with explicit references to the `skills/<slug>/**/codex-agents/*.toml` walk root and `skills/bill-feature-implement/parsing_tolerance.md` for runtime-specific RESULT-block tolerance.
- Predecessors: stacked on `[2026-05-02] multi-runtime-subagents-feature-implement-authoring` (14 subagent files) and `[2026-05-02] multi-runtime-subagents-feature-implement-foundation` (discovery primitive + skills_root wiring + parsing_tolerance.md). Together these three subtasks complete SKILL-33 follow-up 3.
Feature flag: N/A
Acceptance criteria: 8/8 implemented

## [2026-05-02] multi-runtime-subagents-feature-implement-authoring
Areas: skills/bill-feature-implement/codex-agents/, skills/bill-feature-implement/opencode-agents/, .agnix.toml
- Authored 14 native subagent definition files (7 Codex TOML + 7 OpenCode markdown) at `skills/bill-feature-implement/{codex-agents,opencode-agents}/` covering every phase that runs as a subagent today: pre-planning, planning, implementation, implementation-fix (fix-loop variant), completeness-audit, quality-check, pr-description. Each file inlines the corresponding briefing fenced body from `skills/bill-feature-implement/reference.md` VERBATIM, preserving `{placeholder}` tokens (`{feature_name}`, `{plan_json}`, etc.) for orchestrator substitution at spawn time. reusable
- Mirrored canonical Kotlin code-review prior art structurally: Codex TOML uses top-level `name` + `description` + triple-double-quoted `developer_instructions` (mirror `bill-kotlin-code-review-architecture.toml`); OpenCode MD uses unquoted YAML frontmatter (`name`, `description`, `mode: subagent`) followed by a blank line and the briefing body verbatim (mirror `bill-kotlin-code-review-architecture.md`). Filename stem == `name` field for all 14 files. NO F-XXX Risk Register markers (review-specialist-only contract); these workflow-phase subagents return their own briefing-defined `RESULT:` JSON shapes.
- Did NOT wrap OpenCode body in a triple-backtick fence (initial implementation chose to fence-wrap to suppress agnix `xml_balance` warnings on `<placeholder>` tokens; corrected in code-review fix-loop). Instead extended `.agnix.toml` `exclude = [...]` with two new entries: `skills/bill-feature-implement/opencode-agents/**` (briefing placeholders are intentional, not unclosed XML) and `.feature-specs/**` (spec narratives are author-facing design docs and legitimately reference absolute paths and runtime install dirs). reusable
- Foundation from subtask 1 (discovery walks `skills/<skill>/{codex,opencode}-agents/`, runtime-neutral spawn-by-name in reference.md, parsing_tolerance.md Resolution A) carried through unchanged. Existing tests still pass (220/220); the file-shape gate at `tests/test_codex_agents_toml.py` and `tests/test_opencode_agents_md.py` does NOT yet scan the `skills/` tree — subtask 3 will extend discovery and assert universal contracts (filename==name, mode=subagent, no Claude-only tokens) on the new files.
- Branch reused after PR #93 merged; this commit stacks on PR #94 (foundation). PR for this subtask must wait for #94 to merge before landing cleanly on main.
Feature flag: N/A
