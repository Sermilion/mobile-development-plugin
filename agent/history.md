## [2026-04-17] addon-shell-pilot
Areas: orchestration/shell-content-contract/, skill_bill/shell_content_contract.py, skill_bill/scaffold.py, skill_bill/scaffold_manifest.py, platform-packs/kmp/addons/, platform-packs/kmp/platform.yaml, scripts/skill_repo_contracts.py, scripts/validate_agent_configs.py, skills/base/bill-feature-implement/, tests/, README.md, install.sh, uninstall.sh, AGENTS.md
- Promoted governed add-ons onto the shell+content contract via an additive v1.0 extension: `platform.yaml` now accepts a new optional top-level list key `declared_addons` (allowed and required when `governs_addons: true`; forbidden otherwise). Each entry has required `slug`, required `implementation`, required `review`, and optional `topic_files` list. The shell contract version stays `1.0`; packs without `declared_addons` remain contract-compliant. reusable
- Added `AddonDeclaration` dataclass and `OrphanAddonFileError` to `skill_bill/shell_content_contract.py`. Implemented `load_addon_content(pack, slug)` (existence check per file; `MissingContentFileError` on missing, `InvalidManifestSchemaError` on unknown slug; no H2 section enforcement because add-ons are reference material, not skill content) and `scan_orphan_addon_files(pack)` (walks `platform-packs/<slug>/addons/` and raises `OrphanAddonFileError` on unreferenced files). `_build_pack` enforces the `governs_addons` ↔ `declared_addons` cross-check, the kebab-case slug pattern, and duplicate-slug rejection. reusable
- Relocated 12 KMP add-on files via `git mv` from `skills/kmp/addons/` to `platform-packs/kmp/addons/` and removed the now-empty `skills/kmp/` tree. Repointed all 20 runtime sidecar symlinks (7 under `platform-packs/kmp/code-review/bill-kmp-code-review/`, 6 under `platform-packs/kmp/code-review/bill-kmp-code-review-ui/`, 7 under `skills/base/bill-feature-implement/`). `find skills -type l -lname '*skills/kmp/addons*'` returns zero.
- Retired the SKILL-14 TODO at `scripts/skill_repo_contracts.py:18-25`: deleted the hardcoded `GOVERNED_STACK_ADDONS` / `GOVERNED_ADDON_SUPPORT_FILES` / `ADDON_SUPPORTING_FILE_TARGETS` dicts and replaced them with `compute_addon_supporting_file_targets(root)`, which derives sidecar targets from every pack's `declared_addons`. `SUPPORTING_FILE_TARGETS` consumes the computed map. `governed_addon_slugs_for_stack` now reads the pack manifest rather than a hardcoded dict. reusable
- Flipped the scaffolder `add-on` destination from `skills/<platform>/addons/<name>.md` to `platform-packs/<slug>/addons/<name>.md`. Added `skill_bill.scaffold_manifest.append_declared_addon` (mirrors `set_declared_quality_check_file` but operates on the list-shaped key with duplicate-slug rejection). The atomic-rollback transaction already covers this path; added an explicit manifest-write-failure rollback test for add-ons.
- Extended `scripts/validate_agent_configs.py::validate_platform_packs` to iterate every pack with `governs_addons: true`, call `load_addon_content` per declared slug, and run `scan_orphan_addon_files`. Named exceptions surface verbatim; no platform slug enumeration.
- Tests: added seven new fixtures under `tests/fixtures/shell_content_contract/` (addons_valid, addons_missing_file, addons_governs_true_empty, addons_governs_false_declared, addons_slug_only, addons_duplicate_slug, addons_orphan_file) and eight new assertions in `tests/test_shell_content_contract.py` covering happy path, all rejection modes, unknown-slug rejection, and a regression check that the declared_quality_check_file flow stays unaffected. `tests/test_scaffold.py` updated: the existing add-on happy path now asserts the new destination + `declared_addons` append; a new manifest-write-failure rollback test mirrors the SKILL-16 quality-check rollback pattern.
- Runtime wording unchanged: `Selected add-ons:` line, `ADDON_REPORTING_LINE`, `ADDON_IMPLEMENTATION_SUFFIX`, `ADDON_REVIEW_SUFFIX` constants preserved. AGENTS.md/CLAUDE.md updated to pack-owned language; `install.sh` + `uninstall.sh` carry a one-line SKILL-17 documentary comment alongside the existing SKILL-14/16 block (no `RENAMED_SKILL_PAIRS` changes — add-ons are not installable skills).
Feature flag: N/A
